...
def run_inference(model_weights_dir_path, data_dir, output_dir, weights_file_name,
                  use_gpu, gpu_ids, export_png, final_size, training_size, down_steps):
  ...
  print "\nDeep Learning model inference %s"%("using GPU:" if use_gpu else "using CPU:")
  ...
  model = heartseg_model.getUnet3d(down_steps=down_steps, input_shape=inputShape, mgpu=mgpu)
